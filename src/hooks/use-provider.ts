...
export const useProvider: TuseProvider = (redirectUrl) => {
  ...
  const federatedWithPopup = (provider: Providers) => aooth.federatedAuthWithPopup(provider, redirectUrl);
  const federatedWithRedirect = (provider: Providers) => aooth.federatedAuthWithRedirect(provider, redirectUrl);
