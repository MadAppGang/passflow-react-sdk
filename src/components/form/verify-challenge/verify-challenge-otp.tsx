...
export const VerifyChallengeOTP: FC<TVerifyChallengeOTP> = ({
  ...
  const { fetch, fetchPasskey, isError, error, reset } =
  ...
  useEffect(() => {
    if ((otp && challengeId) || valueOTP.length === numInputs) {
      ...
      if (state) {
        payload = {
          ...
          ...(state.identity === "email"
            ? { email: state.identityValue }
            : { phone: state.identityValue }),
        };
      } else {
        payload = {
          create_tenant: createTenant,
      ...
      void (async () => {
        const status =
          state.type === "passwordless"
            ? await fetch(payload)
            : await fetchPasskey(valueOTP, challengeId ?? "");
        if (status) {
          if (!isValidUrl(successAuthRedirect)) navigate(successAuthRedirect);
          else
            window.location.href = getUrlWithTokens(aooth, successAuthRedirect);
        }
      })();
    }
    reset();
  ...
  const onClickResendHandler = () =>
    void refetch(state.passwordlessPayload, "passwordless");
  ...
  return (
    <Navigate
      to={{ pathname: signUpPath, search: localSearchParam.toString() }}
      replace
    />
  );
};

VerifyChallengeOTP.defaultProps = { createTenant: false };
