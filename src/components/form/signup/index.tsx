...
import { useAooth, useAppSettings, useProvider, useSignUp } from "@/hooks";
import {
  cn,
  emailRegex,
  getUrlWithTokens,
...
const initialValues = (state: {
  ...
  if (state) {
    ...
    if (identity && itentityValue)
      return { ...values, [identity]: itentityValue };
  }
  return values;
};
...
const generateIdentityLabel = (
  ...
  if (includes(identities, "username"))
    return useString ? "Use username" : "Username";
  return "";
};
...
export const SignUpForm: FC<TSignUp> = ({
  ...
  const { federatedWithRedirect, federatedWithPopup } =
    useProvider(federatedCallbackUrl);
  ...
  const [validationSchema, setValidationSchema] = useState<ReturnType<
    typeof validationSingUpSchemas
  > | null>(null);

  const [passwordlessExperience, setPasswordlessExperience] =
    useState<boolean>(false);
  const [currentIdentityField, setCurrentIdentityField] =
    useState<PreferIdentity>("none");
  ...
  useLayoutEffect(() => {
    if (appSettings) {
      ...
      const checkHasMainChallenges = some(
        ["email", "phone", "username"],
        (challenge) => includes(IDENTITY_FIELDS, challenge)
      );
      if (
        some(["email", "username"], (identity) =>
          includes(IDENTITY_FIELDS, identity)
        ) &&
        checkHasMainChallenges
      )
        preferredIdentity = "identity";
      else if (includes(IDENTITY_FIELDS, "phone") && checkHasMainChallenges)
        preferredIdentity = "phone";
      else preferredIdentity = "none";
      const identityField =
        preferredIdentity === "identity" ? ["email", "username"] : ["phone"];
      const identityHasPassword = identityField.find((idField: string) =>
        find(INTERNAL[idField], { challenge: "password" })
      );
      if (identityHasPassword) setHasPassword(true);
      else setHasPassword(false);
      if (
        includes(appSettings.CHALLENGES, "passkey") &&
        some(["email", "username", "phone"], (identity) =>
          includes(appSettings.IDENTITY_FIELDS, identity)
        )
      )
  ...
  const onSubmitPasskeyHandler = async (
    values: Omit<ReturnType<typeof initialValues>, "password">
  ) => {
  ...
  const onSubmitPasswordlessHandler =
    (
      field: keyof Pick<AoothPasswordlessSignInPayload, "email" | "phone">,
      value: string,
      type: ChallengeType
    ) =>
    async () => {
  ...
  const onSwitchFieldHandler = (
    ...
    resetForm: (
      nextState?: Partial<FormikState<ReturnType<typeof initialValues>>>
    ) => void
  ) => {
    const identityField =
      field === "identity" ? ["email", "username"] : ["phone"];
    if (appSettings) {
      const { INTERNAL } = appSettings;
      const fieldHasPassword = identityField.find((idField: string) =>
        find(INTERNAL[idField], { challenge: "password" })
      );
  ...
  const onClickProviderHandler = (provider: Providers) =>
    federatedDisplayMode === "redirect"
      ? federatedWithRedirect(provider)
      : federatedWithPopup(provider);

  const passwordlessChallengeButton = (
    ...
    const passwordlessStyle = cn(
      "aooth-m-auto aooth-text-White aooth-text-body-2-semiBold",
      {
        "aooth-mb-[16px]": includes(appSettings.CHALLENGES, "passkey"),
        "aooth-border-Primary !aooth-text-Primary !aooth-text-body-2-medium":
          hasPassword,
      }
    );
    ...
    const { INTERNAL } = appSettings;
    const identityField =
      currentIdentityField === "identity" ? "email" : "phone";
  ...
  if (appSettings) {
    return (
      <Wrapper
        ...
        subtitle="For Aooth by Madappgang"
      >
        {includes(appSettings.CHALLENGES, "passkey") &&
          some(["email", "username", "phone"], (identity) =>
            includes(appSettings.IDENTITY_FIELDS, identity)
                  ...
                  {hasMainChallenges && (
                    <>
                      ...
                      >
                        ...
                        {includes(appSettings.CHALLENGES, "password") &&
                          hasPassword && (
                            <div
        ...
        {size(appSettings.PROVIDERS) > 0 && (
          <div className="aooth-mt-[32px] aooth-px-[24px]">
            ...
            <ProvidersBox
              ...
              onClick={onClickProviderHandler}
            />
          </div>
        )}
      </Wrapper>
