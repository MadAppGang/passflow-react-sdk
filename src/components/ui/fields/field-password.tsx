...
const generateMessage = (
  requires: Record<
    "requireLowerCase" | "requireUpperCase" | "requireSymbol" | "requireNumber",
    boolean
  >
) => {
  ...
  const requirements = Object.keys(requires).filter(
    (key) => requires[key as keyof typeof requires]
  );
  const requirementsString = new Intl.ListFormat("en").format(
    requirements.map(
      (requirement) =>
        requiresString[requirement as keyof typeof requiresString]
    )
  );
...
export const FieldPassword: FC<TFieldPassword> = ({
  ...
  const errorStyles = (fields: string[]) => {
    if (size(value) > 0 && size(intersection(validationErrors, fields)) === 0)
      return "aooth-stroke-Success aooth-text-Success";
    return null;
  };
  ...
  const buttonStyle =
    "aooth-absolute aooth-right-0 aooth-top-1/2 -aooth-translate-y-1/2 -aooth-translate-x-[12px]";
    ...
    <>
      <div className="aooth-relative aooth-w-full">
        <input
          ...
          className={cn(
            styles,
            "aooth-field aooth-field--focused aooth-pr-[40px]",
            className
          )}
          disabled={disabled}
          {...rest}
