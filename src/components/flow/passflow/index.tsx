...
const combineRoutesWithPrefix = (prefix?: string) =>
	Object.keys(routes).reduce(
		(acc, key) => {
			...
			acc[PATH] =
				`${normalizePathPrefix(prefix)}${routes[key as keyof typeof routes]?.path}`;
			return acc;
		},
...
const PassflowWrapper: FC<PassflowProps> = ({
	...
	return (
		<Routes>
			...
			/>
			<Route
				path={routesWithPrefix.verify_magic_link}
				element={<VerifyChallengeMagicLink />}
			/>
			<Route
			...
			/>
			<Route
				path={routesWithPrefix.forgot_password_success}
